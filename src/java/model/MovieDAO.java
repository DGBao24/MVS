...
//                        "WHERE m.MovieID = ?";
    ...
    public List<Movie> getTop4ShowingMovie() {
        List<Movie> list = new ArrayList<>();
        String query = "SELECT TOP (4) \n"
                + "    m.MovieID, m.MovieName, m.Duration, m.Genre, m.Director, \n"
                + "    m.ReleaseDate, m.Description, m.Rate, i.ImagePath, \n"
                + "    m.TrailerURL, m.BasePrice, m.Status \n"
                + "FROM [dbo].[Movie] m \n"
                + "JOIN Image i ON m.MoviePoster = i.ImageID \n"
    ...
    public List<Movie> getTop4UpcomingMovie() {
        List<Movie> list = new ArrayList<>();
        String query = "SELECT TOP (4) \n"
                + "    m.MovieID, m.MovieName, m.Duration, m.Genre, m.Director, \n"
                + "    m.ReleaseDate, m.Description, m.Rate, i.ImagePath, \n"
